...
/// Also used for scalar (non-SIMD) lookup constraint evaluation in the prover.
#[derive(Debug)]
pub struct VerifierConstraintFolder<'a, SC: StarkGenericConfig> {
  ...
  /// Permutation (lookup aux) trace — a pair of consecutive rows.
  /// Each EF value is stored directly (not flattened to base field).
  pub permutation: Option<ViewPair<'a, SC::Challenge>>,
  /// Random challenges used in the permutation / lookup argument.
...
impl<'a, SC: StarkGenericConfig> AirBuilder for ProverConstraintFolder<'a, SC> {
  ...
  fn assert_zeros<const N: usize, I: Into<Self::Expr>>(&mut self, array: [I; N]) {
    ...
    self.accumulator += PackedChallenge::<SC>::from_basis_coefficients_fn(|i| {
      let alpha_powers =
        &self.decomposed_alpha_powers[i][self.constraint_index..(self.constraint_index + N)];
...
// ─── Extension and Permutation builders for VerifierConstraintFolder ───────
...
impl<SC: StarkGenericConfig> ExtensionBuilder for VerifierConstraintFolder<'_, SC> {
  ...
  fn assert_zero_ext<I: Into<Self::ExprEF>>(&mut self, x: I) {
    // Same Horner accumulation as assert_zero but in the extension field.
    self.accumulator *= self.alpha;
    self.accumulator += x.into();
  }
}
...
impl<'a, SC: StarkGenericConfig> PermutationAirBuilder for VerifierConstraintFolder<'a, SC> {
  ...
  fn permutation(&self) -> Self::MP {
    self
      .permutation
      .expect("No permutation trace available — was prove_with_lookup used?")
  }
