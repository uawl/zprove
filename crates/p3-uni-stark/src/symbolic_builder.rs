...
use p3_air::{
  Air, AirBuilder, AirBuilderWithPublicValues, ExtensionBuilder, PairBuilder, PermutationAirBuilder,
};
...
mod tests {
  ...
  fn test_symbolic_air_builder_assert_zero() {
    ...
    assert!(
      constraints
        .iter()
        .any(|x| matches!(x, SymbolicExpression::Constant(val) if *val == BabyBear::new(5))),
