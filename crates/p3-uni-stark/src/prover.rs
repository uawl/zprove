  ...
  Mat: Matrix<Val<SC>> + Sync,
{
  ...
  let mut sels =
    debug_span!("Compute Selectors").in_scope(|| trace_domain.selectors_on_coset(quotient_domain));
...
// ─── Lookup / Permutation argument support ────────────────────────────────

/// Lift a base-field value into the extension field.
#[inline]
fn val_to_ef<SC: StarkGenericConfig>(v: Val<SC>) -> SC::Challenge {
  SC::Challenge::from_basis_coefficients_fn(|d| if d == 0 { v } else { Val::<SC>::ZERO })
}

/// Scalar (non-SIMD) evaluation of lookup constraints across the quotient domain.
///
/// For each position `i` in `0..quotient_size`, builds a [`VerifierConstraintFolder`] with
/// the main trace, optional preprocessed trace, and the permutation (auxiliary) trace, then
/// calls `eval_lookup` to accumulate the lookup constraints via Horner.  Returns one
/// `SC::Challenge` per quotient-domain position: `lookup_constraint_sum * inv_vanishing`.
///
/// # Alpha power convention
/// The caller must pass the *global* `total_constraint_count` to `quotient_values` so that
/// main constraints use alpha powers `alpha^{total-1} … alpha^{lookup_count}`.  This function
/// then starts fresh from zero and applies Horner over the lookup constraints alone, yielding
/// alpha powers `alpha^{lookup_count-1} … alpha^0`.  Summing the two contributions recovers
/// the full polynomial `P(alpha) = Σ alpha^{total-1-k} * constraint_k`.
#[allow(clippy::too_many_arguments)]
fn lookup_quotient_contribution<SC>(
  eval_lookup: &(impl for<'a> Fn(&mut VerifierConstraintFolder<'a, SC>) + Sync),
  public_values: &[Val<SC>],
  trace_domain: Domain<SC>,
  quotient_domain: Domain<SC>,
  main_on_quotient: &(impl Matrix<Val<SC>> + Sync),
  preprocessed_on_quotient: Option<&(impl Matrix<Val<SC>> + Sync)>,
  perm_on_quotient_flat: &(impl Matrix<Val<SC>> + Sync),
  perm_ef_width: usize,
  ...
  SC: StarkGenericConfig,
{
  ...
  (0..quotient_size)
    .map(|i| {
      let i_next = (i + next_step) % quotient_size;

      // Main trace rows lifted to EF.
      let main_local_ef: Vec<SC::Challenge> = main_on_quotient
        .row(i)
        .unwrap()
        .into_iter()
        .map(val_to_ef::<SC>)
        .collect();
      let main_next_ef: Vec<SC::Challenge> = main_on_quotient
        .row(i_next)
        .unwrap()
        .into_iter()
        .map(val_to_ef::<SC>)
      ...
      let main_view = VerticalPair::new(
        RowMajorMatrixView::new_row(&main_local_ef),
        RowMajorMatrixView::new_row(&main_next_ef),
      );

      // Preprocessed rows (optional), lifted to EF.
      let (prep_local_storage, prep_next_storage): (Vec<SC::Challenge>, Vec<SC::Challenge>) =
        if let Some(pp) = preprocessed_on_quotient {
          (
            pp.row(i)
              .unwrap()
              .into_iter()
      ...
      let preprocessed_view = preprocessed_on_quotient.map(|_| {
        VerticalPair::new(
          RowMajorMatrixView::new_row(&prep_local_storage),
          RowMajorMatrixView::new_row(&prep_next_storage),
        )
      });

      // Perm trace rows: reconstruct EF values from flattened base-field columns.
      // Flat layout: [c0_bf0, c0_bf1, c1_bf0, c1_bf1, ...] per row.
      let perm_flat_local: Vec<Val<SC>> =
        perm_on_quotient_flat.row(i).unwrap().into_iter().collect();
      let perm_flat_next: Vec<Val<SC>> = perm_on_quotient_flat
        .row(i_next)
        .unwrap()
        .into_iter()
        .collect();
      let perm_local_ef: Vec<SC::Challenge> = (0..perm_ef_width)
        .map(|c| SC::Challenge::from_basis_coefficients_fn(|k| perm_flat_local[c * d + k]))
        .collect();
      let perm_next_ef: Vec<SC::Challenge> = (0..perm_ef_width)
        .map(|c| SC::Challenge::from_basis_coefficients_fn(|k| perm_flat_next[c * d + k]))
        .collect();
      let perm_view = VerticalPair::new(
        RowMajorMatrixView::new_row(&perm_local_ef),
        RowMajorMatrixView::new_row(&perm_next_ef),
      );

      let mut folder = VerifierConstraintFolder {
        main: main_view,
        preprocessed: preprocessed_view,
        permutation: Some(perm_view),
        permutation_challenges: perm_challenges,
        public_values,
        is_first_row: val_to_ef::<SC>(sels.is_first_row[i]),
        is_last_row: val_to_ef::<SC>(sels.is_last_row[i]),
        is_transition: val_to_ef::<SC>(sels.is_transition[i]),
        alpha,
        accumulator: SC::Challenge::ZERO,
      };
      eval_lookup(&mut folder);

      folder.accumulator * val_to_ef::<SC>(sels.inv_vanishing[i])
    })
    .collect()
}

/// Generates a STARK proof with a permutation/lookup argument (e.g. LogUp).
///
/// Compared to [`prove_with_preprocessed`], this function:
/// 1. Samples `num_perm_challenges` permutation challenges *after* committing the main trace.
/// 2. Calls `perm_trace_fn` with those challenges; if it returns `Some`, commits the auxiliary
///    trace (flattened to base field).
/// 3. Mixes lookup constraints into the quotient polynomial by calling `eval_lookup` for each
///    position on the quotient domain (scalar loop).
/// 4. Includes the perm commitment / opening in the proof.
///
/// # Type parameters
...
>(
  ...
  public_values: &[Val<SC>],
  preprocessed: Option<&PreprocessedProverData<SC>>,
  // Called with sampled perm challenges; returns optional EF-valued auxiliary trace.
  perm_trace_fn: impl FnOnce(&[SC::Challenge]) -> Option<RowMajorMatrix<SC::Challenge>>,
  // How many permutation challenges to sample (e.g. 2 for LogUp beta/gamma).
  num_perm_challenges: usize,
  // How many constraints the lookup gadget adds (for correct alpha-power offset).
  ...
  A: Air<SymbolicAirBuilder<Val<SC>>> + for<'a> Air<ProverConstraintFolder<'a, SC>>,
{
  ...
  let preprocessed_width = preprocessed.map_or_else(
    || {
      if let Some(preprocessed_trace) = air.preprocessed_trace() {
        ...
        if width > 0 {
          panic!(
            "AIR defines preprocessed columns (width = {}), but no \
                         PreprocessedProverData was provided.",
            width
          );
        }
      }
    ...
    |pp| {
      assert_eq!(
        pp.degree_bits, log_ext_degree,
        "PreprocessedProverData degree_bits mismatch"
      );
      pp.width
    },
  );
  ...
  // Observe the count so transcript diverges if num_perm_challenges differs.
  ...
  // ── generate + commit permutation trace ──────────────────────────────────
  let perm_trace_opt = perm_trace_fn(&perm_challenges);
  let (perm_commit_opt, perm_data_opt, perm_ef_width) = if let Some(perm_ef) = perm_trace_opt {
    let perm_ef_width = perm_ef.width();
    let d = SC::Challenge::DIMENSION;
    // Flatten extension-field rows into base-field columns.
    let flat_values: Vec<Val<SC>> = perm_ef
      .values
      .iter()
      .flat_map(|ef| ef.as_basis_coefficients_slice().iter().copied())
      .collect();
    let perm_flat = RowMajorMatrix::new(flat_values, perm_ef_width * d);
    let (commit, data) =
      info_span!("commit to perm trace").in_scope(|| pcs.commit([(ext_trace_domain, perm_flat)]));
    challenger.observe(commit.clone());
    (Some(commit), Some(data), perm_ef_width)
  } else {
    (None, None, 0)
  };
  ...
  // ── lookup quotient contribution (scalar loop) ────────────────────────────
  if let Some(ref perm_data) = perm_data_opt {
    let perm_on_quotient_flat = pcs.get_evaluations_on_domain(perm_data, 0, quotient_domain);
    let lookup_contrib = lookup_quotient_contribution(
      &eval_lookup,
      public_values,
      trace_domain,
      quotient_domain,
      &trace_on_quotient_domain,
      preprocessed_on_quotient_domain.as_ref(),
      &perm_on_quotient_flat,
      perm_ef_width,
      &perm_challenges,
      alpha,
    );
    for (q, l) in quotient_vals.iter_mut().zip(lookup_contrib.iter()) {
      *q += *l;
    }
  }
  ...
  let (opened_values, opening_proof) = info_span!("open").in_scope(|| {
    ...
    let round3 = preprocessed_data_ref.map(|d| (d, vec![vec![zeta, zeta_next]]));
    let round4 = perm_data_opt
      .as_ref()
      .map(|d| (d, vec![vec![zeta, zeta_next]]));

    let rounds: Vec<_> = round0
      .into_iter()
      .chain([round1, round2])
      .chain(round3)
      .chain(round4)
  ...
  // Perm round comes after preprocessed (if any).
  let (perm_local, perm_next) = if has_perm {
    let perm_round_idx = SC::Pcs::QUOTIENT_IDX + 1 + (preprocessed_width > 0) as usize;
    (
      Some(opened_values[perm_round_idx][0][0].clone()),
      Some(opened_values[perm_round_idx][0][1].clone()),
    )
  } else {
    (None, None)
  };
